...
# ============================================================
# In Chromium-based browsers (Chrome, Edge, etc.), the HTML5 "download"
# attribute used by shiny::downloadButton() can conflict with Shinylive.
# The symptom is a failed download named like download_xxx.txt / "No file".
# Removing the attribute lets Shinylive intercept the download request
# and transfer the file from webR's virtual filesystem to the browser.
...
APP_UPDATED <- "Last updated on September 9, 2026"
...
# ============================================================
# ROBUST EXPORT HELPERS - SESSION TEMP FILE + downloadHandler
# ============================================================
...
generated_download_ui <- function(result, download_id, button_label, icon_name = "download") {
  if (is.null(result)) {
    return(tags$p(class = "small-note", "Click Generate first. After the file is successfully created and verified, the download button will appear."))
  }
  if (!isTRUE(result$ok)) {
      ...
      tabPanel(
        ...
        fluidRow(
          box(width = 12, title = "Export Settings and Captions", status = "primary", solidHeader = TRUE,
              ...
              tags$p(class = "small-note", "Generated files are first created in a writable session temporary folder, verified, and then delivered through Shiny downloadHandler. This is more reliable than writing dynamically to www/."))
        ),
        fluidRow(
...
server <- function(input, output, session) {
  ...
  output$download_chart_generated <- downloadHandler(
    filename = function() { req(chart_export_result()); chart_export_result()$filename },
    contentType = "image/png",
    content = function(file) {
      res <- chart_export_result(); req(res, isTRUE(res$ok), file.exists(res$file)); file.copy(res$file, file, overwrite = TRUE)
    }
  )
  output$download_excel_generated <- downloadHandler(
    filename = function() { req(excel_export_result()); excel_export_result()$filename },
    contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    content = function(file) {
      res <- excel_export_result(); req(res, isTRUE(res$ok), file.exists(res$file)); file.copy(res$file, file, overwrite = TRUE)
    }
  )
  output$download_word_generated <- downloadHandler(
    filename = function() { req(word_export_result()); word_export_result()$filename },
    contentType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    content = function(file) {
      res <- word_export_result(); req(res, isTRUE(res$ok), file.exists(res$file)); file.copy(res$file, file, overwrite = TRUE)
    }
  )

  output$download_chart_fallback <- downloadHandler(
    filename = function() make_export_filename("statcal_likert_stacked_bar_chart", "png", input$export_dpi),
    contentType = "image/png",
    content = function(file) {
      generate_plot_png(stacked_bar_plot_object(), file, input$export_width, input$export_height, input$export_dpi, safe_theme_bg(input$chart_theme))
    }
  )
  output$download_excel_fallback <- downloadHandler(
    filename = function() make_export_filename("statcal_likert_analysis", "xlsx"),
    contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    content = function(file) export_current_excel(file)
  )
  output$download_word_fallback <- downloadHandler(
    filename = function() make_export_filename("statcal_likert_analysis_report", "docx"),
    contentType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
